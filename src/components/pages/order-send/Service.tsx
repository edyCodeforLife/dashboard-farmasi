...
const Service = () => {
  ...
  const { getOrderSend, orderArrived } = ServiceOrderSend();
  const fileLocation = 'components/pages/order-send/Service.tsx';
  ...
  const getListOrder = async () => {
    try {
      ...
      const result: any = await getOrderSend(params);
  ...
  const arrivedHandlerApi = async (id: number) => {
    try {
      const arrived = await orderArrived(id);
      setWarning({ message: 'Berhasil mengubah status pesanan menjadi Tiba di Tujuan', type: 'success' });
      setMeta({ ...meta, page: 1 });
      setDisabled(false);
    } catch (error: any) {
      errorCatcher({ error, file: fileLocation, func: 'arrivedHandlerApi' });
      setWarning({ message: `Order Arrived: ${error?.data?.message || error?.message}`, type: 'error' });
      setDisabled(false);
    }
  };

  const arrivedHandler = (id: number) => {
    setDisabled(true);
    arrivedHandlerApi(id);
  };
  ...
  useEffect(() => SetStorage('currentUrl', '/order-send'), []);
