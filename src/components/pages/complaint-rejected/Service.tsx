...
import ServiceComplaintRejected from '@services/api/ServiceComplaintRejected';
...
const Service = () => {
  ...
  const { getComplaintRejected } = ServiceComplaintRejected();
  const fileLocation = 'components/pages/complaint-rejected/Service.tsx';
  ...
  const getListComplaint = async () => {
    try {
      ...
      const result: any = await getComplaintRejected(params);
  ...
  useEffect(() => SetStorage('currentUrl', '/complaint-rejected'), []);
