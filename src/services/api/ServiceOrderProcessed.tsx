...
const ServiceOrderProcessed = () => {
  ...
  const getOrderProcessed = (params: any) => new Promise((resolve: any, reject: any) => {
    Api.get(`${urlService}/order-processed`, {
  ...
  const pickupOrder = (params: any, id: number) => new Promise((resolve: any, reject: any) => {
    Api.post(`${urlService}/order-processed/pickup/${id}`, params, {
      headers: { Authorization: `Bearer ${GetStorage('access_token')}` },
    }).then((response: any) => {
      resolve(response?.data?.data);
    }).catch((error: any) => {
...
export default ServiceOrderProcessed;
