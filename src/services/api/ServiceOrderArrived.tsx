...
const ServiceOrderArrived = () => {
  ...
  const getOrderArrived = (params: any) => new Promise((resolve: any, reject: any) => {
    Api.get(`${urlService}/order-arrived`, {
  ...
  const makeComplaint = (params: any, id: number) => new Promise((resolve: any, reject: any) => {
    Api.post(`${urlService}/order-arrived/complaint/${id}`, params, {
      headers: { Authorization: `Bearer ${GetStorage('access_token')}` },
    }).then((response: any) => {
      resolve(response?.data?.data);
    }).catch((error: any) => {
...
export default ServiceOrderArrived;
