...
const ServiceOrderAccepted = () => {
  ...
  const getOrderAccepted = (params: any) => new Promise((resolve: any, reject: any) => {
    Api.get(`${urlService}/order-accepted`, {
  ...
  const processOrder = (params: any, id: number) => new Promise((resolve: any, reject: any) => {
    Api.post(`${urlService}/order-accepted/process/${id}`, params, {
      headers: { Authorization: `Bearer ${GetStorage('access_token')}` },
    }).then((response: any) => {
      resolve(response);
    }).catch((error: any) => {
...
export default ServiceOrderAccepted;
