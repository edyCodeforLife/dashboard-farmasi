...
const ServiceOrderSend = () => {
  ...
  const getOrderSend = (params: any) => new Promise((resolve: any, reject: any) => {
    Api.get(`${urlService}/order-send`, {
  ...
  const orderArrived = (id: number) => new Promise((resolve: any, reject: any) => {
    Api.get(`${urlService}/order-send/arrived/${id}`, {
      headers: { Authorization: `Bearer ${GetStorage('access_token')}` },
    }).then((response: any) => {
      resolve(response?.data?.data);
    }).catch((error: any) => {
...
export default ServiceOrderSend;
